...
echo "----------------------------------------------------"
echo "Starting Node.js Deploy Process"
echo "----------------------------------------------------"

ARTIFACT_PATH="/var/jenkins_home/artifacts/node_app.tar.gz"
...
TARGET_HOST="node-prod-env"
TARGET_DIR="/home/deployer/node_app"
...
sshpass -p "$TARGET_PASS" ssh -o StrictHostKeyChecking=no "$TARGET_USER@$TARGET_HOST" \
    "cd $TARGET_DIR && tar -xzf app.tar.gz && nohup node index.js > /tmp/app.log 2>&1 &"
